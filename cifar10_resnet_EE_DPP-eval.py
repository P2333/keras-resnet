...
#Load model
model = load_model(filepath, custom_objects={'Loss_withEE_DPP': Loss_withEE_DPP,
...
np.savetxt(os.path.join(save_results_dir, 'test_predictions_models%s_lamda%s_logdetlamda%s_epoch%d.txt'%(str(FLAGS.num_models),str(FLAGS.lamda),str(FLAGS.log_det_lamda),FLAGS.epoch)), predictions)
