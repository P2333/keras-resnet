...
if int(FLAGS.lamda) == 0:
    ...
    model.compile(
        loss=Loss_withEE_plus_leave_one,
        optimizer=Adam(lr=lr_schedule(0)),
        metrics=[acc_metric, Ensemble_Entropy_metric, non_ME_metric])
model.summary()
print(model_type)
...
save_dir = os.path.join(os.getcwd(), 'EEplus_saved_models'+str(FLAGS.num_models)+'_lamda'+str(FLAGS.lamda)+'_nonMElamda'+str(FLAGS.nonME_lamda)+'_leaveone_'+str(FLAGS.augmentation))
