...
def resnet_v1(input, depth, num_classes=10):
...
def Loss_withEE(y_true, y_pred, num_model=2):
    y_p_1, y_p_2 = tf.split(y_pred, num_model, axis=-1)
    y_t_1, y_t_2 = tf.split(y_true, num_model, axis=-1)
    ...
    return CE_1 + CE_2 - FLAGS.lamda * EE
...
if int(FLAGS.lamda) == 0:
    ...
    metrics=[acc_metric, Ensemble_Entropy_metric])
else:
...
save_dir = os.path.join(os.getcwd(), 'EE_saved_models_lamda'+str(FLAGS.lamda)+'_'+str(FLAGS.augmentation))
