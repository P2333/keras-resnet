...
import keras
from keras.layers import AveragePooling2D, Input, Flatten, Activation
...
if version == 2:
    for i in range(FLAGS.num_models):
        ...
        model_out_logits.append(model_dic[str(i)][3])
else:
    for i in range(FLAGS.num_models):
...
if version == 2:
    for i in range(FLAGS.num_models):
        ...
        model_out_logits_baseline.append(model_dic_baseline[str(i)][3])
...
print(model_ensemble_logits)
#Get individual models
wrap_ensemble = KerasModelWrapper(model_ensemble_logits)
wrap_ensemble_baseline = KerasModelWrapper(model_ensemble_logits_baseline)
...
for eps in range(11):
    ...
    preds = model_ensemble_logits(adv_x)
    preds_baseline = model_ensemble_logits_baseline(adv_x_baseline)
